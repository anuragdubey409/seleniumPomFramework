...
public class SynchronizedWaits {
	...
	public void waitForJQueryToLoad() {
		...
		ExpectedCondition<Boolean> jQuery = new ExpectedCondition<Boolean>() {
			public Boolean apply(WebDriver driver) {
				return (Boolean) jsOps.execute("return window.jQuery != undefined && jQuery.active == 0");
			}
		};
		wait.until(jQuery);
	}

	public void waitForAngularJsToComplete() {
		wait = new WebDriverWait(driver, pageLoadTimeOut);
		ExpectedCondition<Boolean> angular = new ExpectedCondition<Boolean>() {
			public Boolean apply(WebDriver driver) {
				return (Boolean) jsOps.execute(
	 ...
	 * @param title
	 */
	public void waitForTheTitleContains(String title) {
		wait=new WebDriverWait(driver, elementVisiblity);
